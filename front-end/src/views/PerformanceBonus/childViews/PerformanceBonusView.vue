        ...
        <el-form-item label="绩效加分类型" prop="PBType">
          <el-cascader
            ...
            <template slot-scope="{ node, data }">
              <el-tooltip :disabled="data.PBType.length < 13"
                          class="item"
                          effect="dark"
                          :content="data.PBType"
                          placement="top-start">
                <span>{{ data.PBType }}</span>
              </el-tooltip>
            </template>
          </el-cascader>
        </el-form-item>
                ...
                <el-table :data="scope.row.PBScoreAssign">
                  ...
                  <el-table-column label="分数" align="center">
                    <template slot-scope="scope2">
                      ...
                      <span v-else>
                        <el-input-number size="mini"
                                        :max="scope.row.PBScoretiaozheng"
                                        v-model="scope2.row.assignScore"
                                        :step="0.1"
                                        :precision="2"
                                        @change="handlePBScoreChange(scope.row)">
                  ...
                  <el-table-column label="操作" align="center">
                    <template slot-scope="scope2">
                      <el-button type="danger"
                                 size="mini"
                                 @click="handleDelAssist(scope.row, scope.row.PBScoreAssign, scope.row.participant, scope2.$index, scope2.row)"
                                 :disabled="scope2.row.role === 1">删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
...
<script>
  ...
  import { getPMBRules, getPMBRulesByID, submitPBData } from '@/utils/performanceBonus'
  ...
  export default {
    ...
    methods: {
      // 初始化
      init () {
        if (isUndefined(Cookies.get('pmbadd'))) {
          this.formData.title = this.$moment().format('YYYY-MM')
          Cookies.set('pmbadd', this.formData.title)
        }
        this.formData.title = Cookies.get('pmbadd')
      ...
      handlekValueChange (row) {
        let oriPBScore = row.PBScore
        row.PBScoretiaozheng = oriPBScore * row.kValue
        row.assignedPBScore = row.PBScoretiaozheng
        row.PBScoreAssign[0].assignScore = row.assignedPBScore
      },

      // **绩效加分类型变化
      PBTypeChange (selectItem) {
        ...
        if (selectLen > tableLen) { // **选中的数目大于列表中的数目
          getPMBRulesByID(difference).then(res => {
            for (let item of res) {
              let obj = {
                id: item.id,
                PBType: item.PBType,
                PBRule: item.PBRule,
                PBRuleID: item.id,
                PBScore: item.PBScore,
                PBComments: '',
                isContinusBonus: item.isContinusBonus,
                continusMonths: item.continusMonths,
                PBScoreAssign: [],
                participant: [],
                assignedPBScore: item.PBScore,
                kValue: 1,
                PBScoretiaozheng: item.PBScore
              }
              let PBscoreAssign = {
                ...
                role: 1,
                assignScore: obj.PBScore,
                deleteAble: false
              }
              obj.PBScoreAssign.push(PBscoreAssign)
              this.PBData.push(obj)
            }
          })
        } else { // 选中的数目小于列表中的数目
          ...
          for (let diff of difference) {
            for (let i = 0; i < this.PBData.length; i++) {
              if (this.PBData[i].id === diff) {
                deleteIndex = i
                break
              }
            }
            this.PBData.splice(deleteIndex, 1)
          }
        }
      },
      ...
      // ***删除协助者按钮
      handleDelAssist (row, PBScoreAssign, participant, index, row2) {
        ...
        this.$refs[formData].validate((valid) => {
          if (valid) {
            ...
            if (isOverPMScore) {
              ...
              let kValue = []
              for (let i = 0; i < this.PBData.length; i++) {
                PBRuleIDs.push(this.PBData[i].id)
                PBScoreAssign.push(this.PBData[i].PBScoreAssign)
                PBComments.push(this.PBData[i].PBComments)
                kValue.push(this.PBData[i].kValue)
              }
              submitPBData(this.formData.PBName,

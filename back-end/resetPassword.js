 ...
 *
 * 说明：
 *   1) 存量 MD5 账号已支持「首次登录无缝自动升级」，通常无需本脚本；
 *      本脚本用于管理员主动改密／强制重置（如忘密、接管账号）。
 *   2) 新密码摘要与前端 computePasswordDigest 完全一致：
 *      PBKDF2-HMAC-SHA256（10 万次迭代，盐 = SHA256(账号)），落库前再经 scrypt 加固，
 *      确保与登录/改密链路的校验约定匹配（传明文或 md5 均会导致无法登录）。
 *   3) 默认要求密码强度：8 位以上且至少包含 3 类字符（大写/小写字母、数字、符号）。
 *   4) 脚本直接写库、不带登录鉴权，仅允许在受控服务器上运行，勿暴露到公网。
 */
